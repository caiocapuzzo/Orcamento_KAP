...
#include "ui_orcamento_editar.h"
#include "dados_do_cliente.h"
#include "editar_item.h"


 QString e_qtde_e, e_codp_e;
 double Desconto_e = 0;
 double Desconto_e_excel_e = 0;
double e_livre_e = 0;
double e_tramite_e = 0;
int e_linha_e = 0;
QString m_sigla_e;

orcamento_editar::orcamento_editar(SessaoUsuario sessao, QWidget *parent)
    : QDialog(parent)
    , ui(new Ui::orcamento_editar)
    , m_dados(sessao) // Copia o pacote recebido para a variável m_dados
{
    ...
    QSqlQuery qryedit(InfokapDB) ;
    qryedit.prepare("SELECT * FROM Cotacoes"
                    " WHERE NdaCotacao = :e_id"
                    " AND Deletado IS NULL");
    qryedit.bindValue(":e_id",QString::number(sessao.id));
    if(!qryedit.exec()){
        QMessageBox::warning(this,"FALHA","Falha consulta qryedit");
        return;
    }

    if(qryedit.next()){
        QString aux_cf,aux_dll;
        ui->txt_NdaCotacao->setText(qryedit.value(0).toString());
        ui->cb_dll->setCurrentText(qryedit.value(8).toString());
        ui->txt_uf->setText(qryedit.value(21).toString());
        ui->txt_obs->setText(qryedit.value(11).toString());
        ui->txt_loja->setText(qryedit.value(2).toString());
        ui->txt_tipo->setText(qryedit.value(35).toString());
        ui->txt_codCli->setText(qryedit.value(1).toString());
        ui->txt_cnpjCli->setText(qryedit.value(34).toString());
        ui->txt_nomeCli->setText(qryedit.value(33).toString());
        ui->txt_desconto->setText(qryedit.value(12).toString());
        //ui->txt_p_bairro->setText(qryedit.value(20).toString());
        //ui->txt_p_cidade->setText(qryedit.value(21).toString());
        ui->txt_prospect->setText(qryedit.value(37).toString());
        //ui->txt_cpf_p->setText(qryedit.value(40).toString());
        ui->txt_regiao->setText(qryedit.value(41).toString());
        ui->txt_frete->setText(brasilLocale.toString(qryedit.value(42).toDouble(),'f',2));

        QString Codprospect = qryedit.value(3).toString();
        ui->txt_codprospect->setText(Codprospect);

        if (Codprospect != "0"){
            ui->txt_nomeCli->setText("Orcamento");
            ui->txt_codCli->setEnabled(false);
            ui->bt_ultimasVendas->setEnabled(false);
            on_txt_codprospect_editingFinished();
        }

        ui->txt_contato_1->setText(qryedit.value(15).toString());
        ui->txt_Emailcontato_1 ->setText(qryedit.value(16).toString());
        ui->txt_ddd->setText(qryedit.value(17).toString().left(3));
        ui->txt_telefonecontato_1->setText(qryedit.value(17).toString().simplified().right(9) );
        //ui->txt_p_endereco->setText(qryedit.value(18).toString());

        ui->txt_celularcontato_1->setText(qryedit.value(27).toString());
        ui->cb_atendente->setCurrentText(sessao.sigla);
        m_sigla_e = qryedit.value(6).toString();

        QString frete = qryedit.value(32).toString();
        if (frete=="F"){
            frete="FOB";
        }else{
            frete="CIF";
        }
        ui->cb_frete->setCurrentText(frete);
        QString data1 = (qryedit.value(4).toString().left(4));
        QString data2 = (qryedit.value(4).toString().mid(5,2));
        QString data3 = (qryedit.value(4).toString().mid(8,2));
        //ui->txt_data->setText(data3+"/"+data2+"/"+data1);
        aux_cf=(brasilLocale.toString(qryedit.value(7).toDouble(),'f',2));
        std::replace(aux_cf.begin(),aux_cf.end(),'.',',');
        ui->cb_cf->setCurrentText(aux_cf);
        ui->cb_dll->setCurrentText(qryedit.value(8).toString());
        //ui->txt_vp->setText(qryedit.value(22).toString());

        int chx1 = qryedit.value(10).toInt();
        int chx2 = qryedit.value(13).toInt();
        int chx3 = qryedit.value(14).toInt();
        int chx4 = qryedit.value(30).toInt();
        int chx5 = qryedit.value(31).toInt();
        ...
        QSqlQuery qryeditp(InfokapDB) ;
        qryeditp.prepare("SELECT * FROM CotacoesProdutos "
                         "WHERE NdaCotacao = :id "
                         "AND Deletado IS NULL "
                         "ORDER BY nitem");
        qryeditp.bindValue(":id",sessao.id);
        if(!qryeditp.exec()){
            QMessageBox::warning(this,"FALHA","Falha na consulta qryeditp");
            return;
        }
        ...
        while (qryeditp.next()) {
            ...
            ui->tw_produtos->setItem(cont,1,new QTableWidgetItem(qryeditp.value(1).toString()));    //código
            ui->tw_produtos->setItem(cont,2,new QTableWidgetItem(qryeditp.value(14).toString()));   //código no cliente
            ui->tw_produtos->setItem(cont,3,new QTableWidgetItem(qryeditp.value(3).toString()));    //descrição
            ui->tw_produtos->setItem(cont,4,new QTableWidgetItem(brasilLocale.toString(qryeditp.value(13).toDouble(),'f',2))); //deconto em R$
            ui->tw_produtos->setItem(cont,5,new QTableWidgetItem(brasilLocale.toString(qryeditp.value(5).toDouble(),'f',2)));  // preço unitário
            ui->tw_produtos->setItem(cont,6,new QTableWidgetItem(qryeditp.value(4).toString()));    //qtde
            ui->tw_produtos->setItem(cont,7,new QTableWidgetItem(qryeditp.value(8).toString()));    //prazo
            ui->tw_produtos->setItem(cont,8,new QTableWidgetItem(qryeditp.value(6).toString()));    //ipi
            ui->tw_produtos->setItem(cont,9,new QTableWidgetItem(brasilLocale.toString(qryeditp.value(7).toDouble(),'f',2))); //valor total
            ui->tw_produtos->setItem(cont,10,new QTableWidgetItem(qryeditp.value(12).toString()));  //ncm
            ui->tw_produtos->setItem(cont,11,new QTableWidgetItem(brasilLocale.toString(qryeditp.value(15).toDouble(),'f',2))); //st


            //Recuperando o preço cheio - início
            QString codigoKap = qryeditp.value(1).toString().toUpper();
            QSqlDatabase ProtheusDB = QSqlDatabase::database("dbp");
            QSqlQuery qryvu(ProtheusDB);  //preço de venda
            qryvu.prepare("SELECT * FROM DA1010 "
                          "WHERE DA1_CODPRO = :codigoKap "
                          "AND DA1_CODTAB = '001' "
                          "AND D_E_L_E_T_ <> '*' ");
            qryvu.bindValue(":codigoKap",codigoKap);
            if(!qryvu.exec()){
                QMessageBox::warning(this,"FALHA","Falha na consulta qryvu");
                return;
            }
            qryvu.first();
            double precoUnit=qryvu.value(4).toDouble();
            ui->tw_produtos->setItem(cont,12,new QTableWidgetItem(brasilLocale.toString(precoUnit,'f',2))); //preço cheio
            //Recuperando o preço cheio - fim
            ui->tw_produtos->setItem(cont,13,new QTableWidgetItem(qryeditp.value(17).toString()));  //importado?
            ui->tw_produtos->setItem(cont,14,new QTableWidgetItem(qryeditp.value(11).toString()));  //imagem
            ui->tw_produtos->setItem(cont,15,new QTableWidgetItem(qryeditp.value(16).toString()));  //peso bruto
            ui->tw_produtos->setItem(cont,16,new QTableWidgetItem(qryeditp.value(10).toString()));  //imprimi
            ui->tw_produtos->setItem(cont,17,new QTableWidgetItem(qryeditp.value(21).toString()));  //código no concorrente
            ui->tw_produtos->setItem(cont,18,new QTableWidgetItem(qryeditp.value(22).toString()));  //icms
                                                                                                                                                //Código no fornecedor
            ui->tw_produtos->setItem(cont,20,new QTableWidgetItem(qryeditp.value(23).toString()));  //serie
            qDebug() << "Serie "+qryeditp.value(23).toString();
            ui->tw_produtos->setRowHeight(cont,10);   //altura da linha
            //Criando as caixas para selecionar itens
            QTableWidgetItem *selecionado = new QTableWidgetItem();
            ui->tw_produtos->setItem(cont,0, selecionado);
            selecionado->setCheckState(Qt::Unchecked);
            cont++;
        }
    ...
    ui->tw_produtos->setColumnCount(21);
    ui->tw_produtos->setColumnWidth(0,10);
    ui->tw_produtos->setColumnWidth(1,100);
    ui->tw_produtos->setColumnWidth(2,120);
    ui->tw_produtos->setColumnWidth(3,210);
    ui->tw_produtos->setColumnWidth(4,70);
    ui->tw_produtos->setColumnWidth(5,60);
    ui->tw_produtos->setColumnWidth(6,50);
    ui->tw_produtos->setColumnWidth(7,60);
    ui->tw_produtos->setColumnWidth(8,40);
    ui->tw_produtos->setColumnWidth(9,80);
    ui->tw_produtos->setColumnWidth(10,70);
    ui->tw_produtos->setColumnWidth(11,45);
    ui->tw_produtos->setColumnWidth(12,80);
    ui->tw_produtos->setColumnWidth(13,1);
    ui->tw_produtos->setColumnWidth(14,1);
    ui->tw_produtos->setColumnWidth(15,20);
    ui->tw_produtos->setColumnWidth(16,1);
    ui->tw_produtos->setColumnWidth(17,120);
    ui->tw_produtos->setColumnWidth(18,50);
    ui->tw_produtos->setColumnWidth(19,120);
    ui->tw_produtos->setColumnWidth(20,15);
    QStringList cabecalhos={"E","Código Kap","Código no cliente","Descrição", "Desconto_e\n%","Preço \nunitário","Qtde","Prazo p/\nfaturar",
...
orcamento_editar::~orcamento_editar()
{
    delete ui;
}

void orcamento_editar::on_bt_voltar_clicked()
{
    close();
}


void orcamento_editar::on_txt_nomeCli_editingFinished()
{
...
void orcamento_editar::preencherCamposCliente(const QSqlQuery &q) {
...
void orcamento_editar::configurarCabecalhosCliLoja()
{
...
void orcamento_editar::on_txt_codCli_editingFinished()
{
     cliloja();
}


void orcamento_editar::on_txt_cnpjCli_editingFinished()
{
    ui->bt_importar_excel->setEnabled(true);
...
void orcamento_editar::on_txt_prospect_editingFinished()
{
...
void orcamento_editar::prospect_cliloja()
{
...
void orcamento_editar::on_txt_codprospect_editingFinished()
{
...
void orcamento_editar::on_txt_cnpj_prospect_editingFinished()
{
...
void orcamento_editar::on_txt_cpf_prospect_editingFinished()
{
...
void orcamento_editar::on_txt_qtde_editingFinished()
{
    ...
    e_qtde_e = ui->txt_qtde->text();
}



// APÓS DIGITAR O CÓDIGO DO PRODUTO KAP
void orcamento_editar::on_txt_codP_editingFinished()
{
    ...
    //Criando as caixas para selecionar itens
    QTableWidgetItem *selecionado = new QTableWidgetItem();
    ui->tw_produtos->setItem(contLi,0, selecionado);
    selecionado->setCheckState(Qt::Unchecked);


    ui->tw_produtos->setItem(contLi,1,new QTableWidgetItem(codp)); //código na Kap
    //![10]  //---------------RETIRAR ESPAÇOS DENTRO DO CÓDIGO ---------------//


    //![20] //---------- CASO TENHA DIGITADO O CÓDIGO DO PRODUTO DO CONCORRENTE -------------//
    QString codNoConcorrente = ui->txt_cod_concorrente->text().left(20);
    ui->tw_produtos->setItem(contLi,17,new QTableWidgetItem(codNoConcorrente)); //código no concorrente
    ui->txt_cod_concorrente->clear();
    ui->tw_produtos->setItem(contLi,17,new QTableWidgetItem(codNoConcorrente));

    QString qtde = ui->txt_qtde->text();
    QTableWidgetItem *qtde_a = new QTableWidgetItem(qtde);
    ui->tw_produtos->setItem(contLi,6,qtde_a);
    ...
    ui->tw_produtos->setItem(contLi,1,new QTableWidgetItem(codp));  // código do produto  --------------------
    ui->tw_produtos->setItem(contLi,20,new QTableWidgetItem(grupo));  // grupo (série)  --------------------
    //![30]   //---------- LENDO VALORES DA TABELA SB1 ----------------//
    ...
    if(qrypro.value(0).toString().isEmpty()) {
        ...
        if(codp.isEmpty()) {
            ...
            ui->tw_produtos->setItem(contLi, 1, new QTableWidgetItem(cod_original));
            ui->tw_produtos->setItem(contLi, 2, new QTableWidgetItem(" "));
            ui->tw_produtos->setItem(contLi, 3, new QTableWidgetItem("Sem descrição"));
            ui->tw_produtos->setItem(contLi, 6, new QTableWidgetItem(ui->txt_qtde->text()));

            // Preenche o restante com "0"
            for(int col : {4,5,7,8,9,10,11,12,14,15,18})
                ui->tw_produtos->setItem(contLi, col, new QTableWidgetItem("0"));

            ui->tw_produtos->setItem(contLi, 13, new QTableWidgetItem("N"));
            ui->tw_produtos->setItem(contLi, 16, new QTableWidgetItem("1"));
            ui->tw_produtos->setItem(contLi, 17, new QTableWidgetItem(" "));

            ui->tw_produtos->setItem(contLi, 19, new QTableWidgetItem(" "));
            ui->tw_produtos->setItem(contLi, 20, new QTableWidgetItem(" "));
    ...
    if(qrycc.next()){
        ui->tw_produtos->setItem(contLi,2,new QTableWidgetItem(qrycc.value(0).toString()));  // código no cliente
    }
    ui->tw_produtos->setItem(contLi,3,new QTableWidgetItem(descricao));  // decrição do produto
    ...
    ui->tw_produtos->setItem(contLi,19,new QTableWidgetItem(cod_no_fornecedor));
    ...
    ui->tw_produtos->setItem(contLi,13,new QTableWidgetItem (importado));
    ...
    if(regiao != "EXT"){
        ...
        ui->tw_produtos->setItem(contLi,8,ipi_a);
        ipi_a->setTextAlignment(Qt::AlignCenter);
    }else{
        ui->tw_produtos->setItem(contLi,8,new QTableWidgetItem("0"));  // EXT não tem IPI
    }
    //![100]   //---------- OBTENDO O VALOR DO IPI - FIM -----------------------------//


    //![110]   //---------- ROTINA DO Desconto_e - INÍCIO -----------------------------//
    ...
    qrydesc1.first();
    QString ACO_CODREG = qrydesc1.value(0).toString().toUpper(); // tabela ACO010 Regra de Desconto_e

    if(Desconto_e_excel_e > 0){
        Desconto_e = Desconto_e_excel_e;
    }else{
        Desconto_e = 0;
    }
    ...
    if(Desconto_e_excel_e == 0){
        if(!ACO_CODREG.isEmpty() ){
            ...
            qrydesc2.first();
            Desconto_e = qrydesc2.value(0).toDouble(); // tabela ACP010 Itens da regra de Desconto_e - fornece o código do Desconto_e
            ...
            if(Desconto_e == 0 ){
                ...
                qrydesc2.first();
                Desconto_e = qrydesc2.value(0).toDouble();
            }
        }
    }

    ui->tw_produtos->setItem(contLi,4,new QTableWidgetItem(QString::number(Desconto_e)));
    double precoUnitComDesconto_e=((100 - Desconto_e) / 100) * precoUnit;
    QTableWidgetItem *precoUnitComDesconto_e_a = new  QTableWidgetItem(brasilLocale.toString(precoUnitComDesconto_e,'f',2));
    ui->tw_produtos->setItem(contLi,5,precoUnitComDesconto_e_a);
    precoUnitComDesconto_e_a->setTextAlignment(Qt::AlignRight);
    //![110]   //---------- ROTINA DO Desconto_e - FIM -------------------------//


    //![120]   //---------- ROTINA DO NCM - INÍCIO ----------------------------//
    QTableWidgetItem *ncm_a = new QTableWidgetItem(ncm);
    ui->tw_produtos->setItem(contLi,10,ncm_a);
    ...
    if(tipo == "S"){
        ...
        if(st == 0){
            ui->tw_produtos->setItem(contLi,11,new QTableWidgetItem("0.00"));
        }else{
            ui->tw_produtos->setItem(contLi,11,new QTableWidgetItem(qry_st.value(0).toString()));
        }
    }else{
        QString st = "0.00";  // para cliente diferente de tipo = S
        ui->tw_produtos->setItem(contLi,11,new QTableWidgetItem(st));
    }
    ...
    ui->tw_produtos->setItem(contLi,18,new QTableWidgetItem(icms));
    ...
    int e_livre_e = ui->txt_estoquelivre->text().toInt();
    int qtde_desejada = ui->txt_qtde->text().toInt();
    int prazo_final = 0;


    if(e_livre_e >= qtde_desejada) {
        B1_PE_p = 0;
        ...
        if(e_tramite_e + e_livre_e < 0){
            necessidade_produzir = qtde_desejada - (e_livre_e);
        }else{
    ...
    if(prazo_final < B1_PE_p){
        if((e_tramite_e + e_livre_e) > qtde.toInt()){
    ...
    ui->tw_produtos->setItem(contLi, 7, new QTableWidgetItem(QString::number(prazo_final)));
    ...
    ui->tw_produtos->setItem(contLi,12,new QTableWidgetItem(QString::number(precoUnit)));  // preço cheio
    QString aux = precoUnitComDesconto_e_a->text().remove('.').replace(',','.') ;
    ...
    ui->tw_produtos->setItem(contLi,9,valorTotal_a);
    valorTotal_a->setTextAlignment(Qt::AlignRight);
    ui->tw_produtos->setItem(contLi,15,new QTableWidgetItem(peso_bruto));   // peso
    ui->tw_produtos->setItem(contLi,16,new QTableWidgetItem("1"));
    ...
    if(ativo == "N" || bloqueado == "1"){
        ...
        // Para a coluna 8,  colocar o "?" no valor do IPI
        if(ui->tw_produtos->item(contLi, 8)) {
            ui->tw_produtos->item(contLi, 8)->setText("?");
        } else {
            ui->tw_produtos->setItem(contLi, 8, new QTableWidgetItem("?"));
            ui->tw_produtos->item(contLi, 8)->setBackground(Qt::red);
        }
    ...
    ui->tw_produtos->setItem(contLi,14,new QTableWidgetItem(imagem_prod2));
...
void orcamento_editar::Sub_somatorio_dos_precos_e_pesos()
{
    ...
    for(int i=0 ; i<qtdeItens; i++) {
        aux=ui->tw_produtos->item(i,5)->text().remove('.').replace(',','.');
        int qtdeiten = ui->tw_produtos->item(i,6)->text().toInt(); //  Qtde do produto orçado
        aux2=ui->tw_produtos->item(i,8)->text().remove('.').replace(',','.');
        w_valorTotal += aux2.toDouble();
        double peso = ui->tw_produtos->item(i,15)->text().toDouble() * qtdeiten;
        W_pesoLiquido += peso;
    }
...
QString orcamento_editar::buscar_cod_kap()
{
    QSqlDatabase ProtheusDB = QSqlDatabase::database("dbp");
    ...
    qry_sa5.bindValue(":codp",e_codp_e);
    if(!qry_sa5.exec()){
...
int orcamento_editar::sub_tempo_TS()                           // Tempo(dias) p/ montar o produto
{
...
void orcamento_editar::on_bt_correios_clicked()
{
...
int orcamento_editar::calcularPrazoEstrutura(QString codPai, int qtdeNecessariaPai, QStandardItemModel *model_estru) {
...
void orcamento_editar::estoqueS()
{
    QString codp = e_codp_e;
    qDebug() << "Codp "+codp;
    ...
    e_livre_e = B2_QATU - B2_QEMP - B2_QPEDVEN;
    e_tramite_e = v_tramite;

    ui->txt_tramite->setText(QString::number(e_tramite_e));
    ui->txt_estoquelivre->setText(QString::number(e_livre_e, 'f', 0));
}

void orcamento_editar::on_bt_desenho_clicked()
{
    QSqlDatabase InfokapDB = QSqlDatabase::database("dbi");
...
void orcamento_editar::on_bt_dadosdo_cliente_clicked()
{
...
void orcamento_editar::on_bt_excluiritem_clicked()
{
    int e = 0;
    int qtdeItensd = ui->tw_produtos->rowCount();
    for (int i = 0; i < qtdeItensd; i++ ){
        if(ui->tw_produtos->item(i,0)->checkState()) {
            e++;
        }
    }

    if(ui->tw_produtos->currentColumn()!=-1){
        QMessageBox::StandardButton opc=QMessageBox::question(this,"Exclusão","Confirma a exclusão "+QString::number(e)+" itens?",QMessageBox::Yes|QMessageBox::No);
        if (opc==QMessageBox::Yes){
            int qtdeItensd = ui->tw_produtos->rowCount();
            for (int i = 0; i < qtdeItensd; i++ ){
                if(ui->tw_produtos->item(i,0)->checkState()) {
                    ui->tw_produtos->removeRow(i);
                    qtdeItensd--;
                    i--;
                    contLi--;
                }
...
void orcamento_editar::on_bt_abrirOP_clicked()
{
...
void orcamento_editar::on_bt_editaritem_clicked()
{
    ...
    if(contLi != -1){
        ...
        s.codkap_item      = ui->tw_produtos->item(contLi, 1)->text();
        s.codnocli_item    = ui->tw_produtos->item(contLi, 2) ? ui->tw_produtos->item(contLi, 2)->text() : "";
        s.descricao_item   = ui->tw_produtos->item(contLi, 3)->text();
        s.desconto_item    = ui->tw_produtos->item(contLi, 4)->text();
        s.precoun_ititem   = ui->tw_produtos->item(contLi, 5)->text();
        s.qtdedo_item      = ui->tw_produtos->item(contLi, 6)->text();
        s.prazo_item       = ui->tw_produtos->item(contLi, 7)->text();
        s.ipi_item         = ui->tw_produtos->item(contLi, 8)->text();
        s.valortotal_item  = ui->tw_produtos->item(contLi, 9)->text();
        s.ncm_item         = ui->tw_produtos->item(contLi, 10)->text();
        s.st_item          = ui->tw_produtos->item(contLi, 11)->text();
        s.precocheio_item = ui->tw_produtos->item(contLi, 12)->text();
        s.import_item = ui->tw_produtos->item(contLi, 13)->text();
        s.cod_imagem_item = ui->tw_produtos->item(contLi, 14)->text();
        s.peso_item = ui->tw_produtos->item(contLi, 15)->text();

        s.codnoconc_item   = ui->tw_produtos->item(contLi, 17)->text();
        s.icms_item       = ui->tw_produtos->item(contLi, 18)->text();
        s.codnoforn_item   = ui->tw_produtos->item(contLi, 19)->text();
        s.grupo_item   = ui->tw_produtos->item(contLi, 20)->text();
        ...
        if(f_editar_item->exec() == QDialog::Accepted) {
            ...
            setTItem(contLi, 1, s.codkap_item);
            setTItem(contLi, 2, s.codnocli_item);
            setTItem(contLi, 3, s.descricao_item);
            setTItem(contLi, 4, s.desconto_item);
            setTItem(contLi, 5, s.precoun_ititem);
            setTItem(contLi, 6, s.qtdedo_item);
            setTItem(contLi, 7, s.prazo_item);
            setTItem(contLi, 8, s.ipi_item);
            setTItem(contLi, 9, s.valortotal_item);
            setTItem(contLi, 10, s.ncm_item);
            setTItem(contLi, 11, s.st_item);
            setTItem(contLi, 13, s.import_item);
            setTItem(contLi, 14, s.cod_imagem_item);
            setTItem(contLi, 15, s.peso_item);
            setTItem(contLi, 17, s.codnoconc_item);
            setTItem(contLi, 18, s.icms_item);
            setTItem(contLi, 19, s.codnoforn_item);
            setTItem(contLi, 20, s.regiao_item);
...
void orcamento_editar::on_bt_oquecomprou_clicked()
{
    // 1. Validação inicial
...
void orcamento_editar::on_bt_ultimasVendas_clicked()
{
    ...
    QString codcli = ui->txt_codCli->text();
    QString loja = ui->txt_loja->text();
    QString codproduto = ui->tw_produtos->item(linhaSelecionada, 1)->text().toUpper().trimmed();
...
void orcamento_editar::on_bt_prazo_iguais_clicked()
{
    ...
    QString pz =ui->tw_produtos->item(e_linha_e,6)->text() ;
...
void orcamento_editar::on_bt_desconto_geral_clicked()
{
    int qtde_row = ui->tw_produtos->rowCount();
    if (qtde_row < 1) {
    ...
    QLocale brasilLocale(QLocale::Portuguese,QLocale::Brazil);
    QString p1 =ui->tw_produtos->item(e_linha_e,3)->text() ;
    p1.replace(',', '.');

    for(int L = 0; L < qtde_row; ++L){
        ui->tw_produtos->setItem(L,4,new QTableWidgetItem(p1));
        ...
        ui->tw_produtos->setItem(L,5,precoUnitComDesconto_a);
        ...
        QTableWidgetItem  *valorTotal_a = new QTableWidgetItem(brasilLocale.toString(valorTotal,'f',2));
        ui->tw_produtos->setItem(L,9,valorTotal_a);
        valorTotal_a->setTextAlignment(Qt::AlignRight);
    }
    Sub_somatorio_dos_precos_e_pesos();
}
...
void orcamento_editar::on_txt_codP_no_cli_editingFinished()
{
...
void orcamento_editar::on_txt_cod_concorrente_editingFinished()
{
     on_similar();
}

void orcamento_editar::on_similar()
{
...
void orcamento_editar::keyPressEvent(QKeyEvent* ke)
{
...
void orcamento_editar::on_tw_produtos_cellClicked(int row, int column)
{
    Q_UNUSED(column);
    Q_UNUSED(row);

    e_linha_e= ui->tw_produtos->currentRow();
    QString codprod=ui->tw_produtos->item(e_linha_e,1)->text().toUpper().trimmed() ;
    e_codp_e = codprod;
    ...
    qrypro.prepare("SELECT * FROM SB1010 "
                   ...
                   "AND D_E_L_E_T_  <>'*' "
                   "ORDER BY B1_COD");
    qrypro.bindValue(":codprod",e_codp_e);
    if(qrypro.exec()){
        qrypro.first();
        ...
        if(qrypro.value(1).toString()!=""){
            ...
            ui->txt_estoquelivre->setText(QString::number(e_livre_e));
...
void orcamento_editar::on_tw_produtos_cellDoubleClicked(int row, int column)
{
    Q_UNUSED(column);
    Q_UNUSED(row);
...
void orcamento_editar::on_bt_salvar_clicked()
{
    QString MMU = ui->txt_NdaCotacao->text();
    QString cab;
    ...
    for (int i = 1; i <= qtde_de_itens; ++i){
        qDebug () << i;
        QString descri = ui->tw_produtos->item(i,3)->text().trimmed();
        if(descri.isEmpty()){
            QMessageBox::information(this,"AVISO","<h3>A Descrição do produto: " + QString::number(i) + " não pode ficar em branco</h3>");
            return;
        }
    }
    ...
    //gravação do cabeçalho editar - início  (primeiro deleta e depois grava novamente com a alteração)
    QSqlQuery qrysalvar(infokapDB);
    qrysalvar.prepare("delete FROM Cotacoes "
                      "WHERE NdaCotacao = :MMU");
    qrysalvar.bindValue(":MMU",MMU);
    qrysalvar.exec();
    ...
    if(m_sigla_e != M06){
        QMessageBox::StandardButton opc=QMessageBox::question(this,"CONFIRMAÇÂO","<h3>Confirma a alteração do orçamento? Vai ficar gravado a sua sigla</h3> ",QMessageBox::No|QMessageBox::Yes);
    ...
    query.prepare("delete FROM CotacoesProdutos"
                  " WHERE NdaCotacao= :mmu");
    query.bindValue(":mmu",MMU);
    if(!query.exec()){
        QMessageBox::warning(this,"Deletar","Erro ao deletar item ....");
        return;
    }
...
bool orcamento_editar::validarCamposObrigatorios()
{
